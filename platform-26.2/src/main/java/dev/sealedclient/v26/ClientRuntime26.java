...
import dev.sealedclient.common.persistence.TickDebouncedSaveController;
import dev.sealedclient.common.profile.ClientProfile;
...
import dev.sealedclient.v26.command.CommandManager26;
import dev.sealedclient.v26.config.ConfigStore26;
import dev.sealedclient.v26.config.PresetApplication26;
import dev.sealedclient.v26.gui.ClientScreen26;
import dev.sealedclient.v26.gui.ClientScreen26Model;
import dev.sealedclient.v26.hud.ClientHud26;
import dev.sealedclient.v26.hud.CombatTargetBridge26;
import dev.sealedclient.v26.hud.HudLayout26;
import dev.sealedclient.v26.hud.HudMetricsBridge26;
import dev.sealedclient.v26.automation.UtilityAutomation26;
...
import dev.sealedclient.v26.movement.MovementActionArbiter26;
import dev.sealedclient.v26.movement.MovementInputAutomation26;
import dev.sealedclient.v26.movement.MovementNetworkTracker26;
import dev.sealedclient.v26.movement.MovementSafetyPolicy26;
import dev.sealedclient.v26.movement.WalkMovementAutomation26;
import dev.sealedclient.v26.integration.BaritoneNavigator26;
import dev.sealedclient.v26.integration.OptionalBaritoneIntegration26;
...
import dev.sealedclient.v26.utility.UtilityActionArbiter26;
...
import dev.sealedclient.v26.visual.VisualOverlayService26;
...
public final class ClientRuntime26 {
    ...
    private List<String> combatModuleIds() {
        return modules.byCategory().getOrDefault(
                dev.sealedclient.common.module.ModuleCategory.COMBAT,
                List.of()
    ...
    private List<String> movementModuleIds() {
        return modules.byCategory().getOrDefault(
                dev.sealedclient.common.module.ModuleCategory.MOVEMENT,
                List.of()
        ).stream().map(module -> module.descriptor().id()).toList();
    }
    ...
    private void tickBaritone(
        ...
        baritoneModuleApplied = status.ownedBySealed();
        if (RuntimeArbitrationPolicy26.baritoneModuleShouldDeactivate(
                status.ownedBySealed(),
                pendingBaritoneTarget != null,
                status.state()
        )) {
            modules.find("baritone_navigator")
    ...
    private boolean applyAutoDisconnect(Minecraft client) {
        ...
        client.getConnection().getConnection().disconnect(
                Component.literal("Sealed Auto Disconnect: health reached 3 hearts")
        );
        return true;
    }
    ...
    public int panic(Minecraft client) {
        ...
        for (RegisteredModule module : modules.all()) {
            if (module.enabled()
                    ...
                    != dev.sealedclient.common.module.ModuleRisk.PASSIVE) {
                try {
                    module.setEnabled(false);
                    disabled++;
                } catch (RuntimeException exception) {
    ...
    private static Identifier id(String path) {
        return Identifier.fromNamespaceAndPath(SealedClient26.MOD_ID, path);
    }
