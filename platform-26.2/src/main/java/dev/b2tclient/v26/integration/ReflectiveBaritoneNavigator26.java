...
final class ReflectiveBaritoneNavigator26 implements BaritoneNavigator26 {
        ...
        try {
            if (cleanupPending && !finishPendingCleanup()) {
                return NavigationResult.failure(
                        "The previous B2T path is still finishing an "
            ...
            if (observation.hasExternalGoal(ownedGoal)) {
                if (lifecycle.owned()) {
                    lifecycle.cancel(
                            "B2T ownership released; another Baritone goal is active"
                    );
                }
                return NavigationResult.failure(
                        "Another Baritone goal is active; B2T did not replace it"
                );
            }
            if (!lifecycle.owned()
                ...
                return NavigationResult.failure(
                        "The previous B2T path has not stopped yet"
                );
            }
            ...
            if (lifecycle.owned()
                ...
                cleanupPending = true;
                lifecycle.cancel(
                        "Could not replace B2T path during an uncancelable movement"
                );
                return NavigationResult.failure(
    ...
    public synchronized NavigationResult pause() {
        ...
        if (!lifecycle.owned()) {
            return NavigationResult.failure(
                    "No B2T-owned navigation to pause"
            );
        }
        if (lifecycle.paused()) {
            return NavigationResult.success(
                    "B2T navigation is already paused"
            );
        }
        try {
            BaritoneAccess26.Observation observation = access.observe();
            if (!observation.exactGoalPresent(ownedGoal)) {
                lifecycle.cancel(
                        "Cannot pause: Baritone goal changed outside B2T"
                );
                cleanupPending = false;
                return NavigationResult.failure(
                        lifecycle.snapshot().detail()
            ...
            if (!lifecycle.pause()) {
                lifecycle.cancel(
                        "B2T path was canceled instead of paused"
                );
                return NavigationResult.failure(
                        lifecycle.snapshot().detail()
                );
            }
            return NavigationResult.success("B2T navigation paused");
        } catch (RuntimeException | LinkageError exception) {
    ...
    public synchronized NavigationResult resume() {
        ...
        if (!lifecycle.paused()) {
            return NavigationResult.failure(
                    "No paused B2T navigation to resume"
            );
        }
        try {
            BaritoneAccess26.Observation observation = access.observe();
            ...
            if (!lifecycle.resume()) {
                return NavigationResult.failure(
                        "B2T navigation cannot be resumed now"
                );
            }
            access.setGoalAndPath(ownedGoal);
            cleanupPending = false;
            return NavigationResult.success("B2T navigation resumed");
        } catch (RuntimeException | LinkageError exception) {
        ...
        if (!lifecycle.owned()) {
            if (cleanupPending) {
                if (finishPendingCleanup()) {
                    return NavigationResult.success(
                            "B2T-owned navigation cancellation completed"
                    );
                }
                return NavigationResult.failure(
                        "B2T path is still finishing an uncancelable movement"
                );
            }
            return NavigationResult.success(
                    "No B2T-owned navigation is active"
            );
        }
        try {
            if (lifecycle.paused()) {
                lifecycle.cancel("B2T navigation stopped");
                cleanupPending = false;
                return NavigationResult.success("B2T navigation stopped");
            }
            ...
            lifecycle.cancel(fullyCancelled
                    ? "B2T navigation stopped"
                    : "B2T navigation is finishing an uncancelable movement");
            return fullyCancelled
                    ? NavigationResult.success("B2T navigation stopped")
    ...
    private NavigationResult stopAfterFailure() {
        ...
        if (finishPendingCleanup()) {
            return NavigationResult.success(
                    "B2T-owned navigation cancellation completed after "
                            + "integration failure"
            );
        }
        return NavigationResult.failure(
