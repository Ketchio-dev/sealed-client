package dev.b2tclient.v26.command;
...
import dev.b2tclient.v26.gui.HudEditorScreen26;
import dev.b2tclient.v26.gui.ProfileScreen26;
...
public final class CommandManager26 {
    private static final String PREFIX = ";b2t";
    ...
    public void initialize() {
        ClientSendMessageEvents.ALLOW_CHAT.register(message -> {
            ...
            try {
                ...
                dev.b2tclient.v26.B2TClient26.LOGGER.error(
    ...
    private void execute(String input) {
        ...
        switch (args[0].toLowerCase(Locale.ROOT)) {
            ...
            case "hud" -> hud(args);
            case "panic" -> panic();
            default -> reply("Unknown command. Use ;b2t help");
        }
    }
    ...
    private void status(String[] args) {
        if (args.length < 2) {
            reply("Usage: ;b2t status <module-id>");
            return;
        }
    ...
    private void toggle(String[] args) {
        if (args.length < 2) {
            reply("Usage: ;b2t toggle <module-id>");
            return;
        }
        RegisteredModule module = runtime.modules().find(args[1]).orElse(null);
        if (module == null) {
    ...
    private void friend(String[] args) {
        ...
        if (args.length < 3) {
            reply("Usage: ;b2t friend <add|remove> <name>");
            return;
        }
        if ("add".equalsIgnoreCase(args[1])) {
            ...
            reply("Usage: ;b2t friend <add|remove|list> [name]");
        }
    }
    ...
    private void waypoint(String[] args) {
        ...
        reply("Usage: ;b2t waypoint <add|remove|list> [name]");
    }
    ...
    private void profile(String[] args) {
        ...
        if (args.length < 3) {
            reply("Usage: ;b2t profile <save|use|delete> <name> [server-pattern]");
            return;
        }
        ...
        if ("save".equalsIgnoreCase(args[1])) {
            ...
            reply("Usage: ;b2t profile <save|use|delete|list|gui> [name] [server-pattern]");
        }
    }
    ...
    private void baritone(String[] args) {
        ...
        if (!"goto".equalsIgnoreCase(args[1]) || args.length != 5) {
            reply("Usage: ;b2t baritone <goto x y z|stop|status>");
            return;
        }
        try {
    ...
    private void hud(String[] args) {
        ...
        reply("Usage: ;b2t hud <edit|reset>");
    }
    ...
    private static void reply(String message) {
        ...
        if (client.player != null) {
            client.player.sendSystemMessage(Component.literal("[B2T] " + message));
        }
    }
}
