...
final class BaritoneApiNavigator implements BaritoneNavigator {
    ...
    public synchronized NavigationResult pause() {
        if (!lifecycle.owned()) {
            return NavigationResult.failure("No B2T-owned navigation to pause");
        }
        if (lifecycle.paused()) {
            return NavigationResult.success("B2T navigation is already paused");
        }
        try {
            IBaritone baritone = primaryAndRegister();
            if (!goalStillOwned(baritone)) {
                lifecycle.cancel("Cannot pause: Baritone goal changed outside B2T");
                return NavigationResult.failure(lifecycle.snapshot().detail());
            }
            if (!lifecycle.pause()) {
                return NavigationResult.failure("B2T navigation cannot be paused now");
            }
    ...
    public synchronized NavigationResult resume() {
        if (!lifecycle.paused()) {
            return NavigationResult.failure("No paused B2T navigation to resume");
        }
        try {
            IBaritone baritone = primaryAndRegister();
            ...
            if (!lifecycle.resume()) {
                return NavigationResult.failure("B2T navigation cannot be resumed now");
            }
    ...
    public synchronized NavigationResult stop() {
        if (!lifecycle.owned()) {
            return NavigationResult.success("No B2T-owned navigation is active");
        }
        try {
            IBaritone baritone = primaryAndRegister();
            boolean wasPaused = lifecycle.paused();
            if (!wasPaused && !goalStillOwned(baritone)) {
                lifecycle.cancel("B2T ownership released; another Baritone goal is active");
