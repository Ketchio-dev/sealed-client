package dev.sealedclient.command;
...
import dev.sealedclient.module.combat.AutoCrystalModule;
...
public final class CommandManager {
    public static final String PREFIX = ";sealed";
    ...
    public boolean execute(String input, Minecraft minecraft) {
        ...
        try {
            ...
            return switch (command) {
                ...
                case "modules", "list" -> listModules(tail);
                default -> {
    ...
    private boolean diagnostics(List<String> arguments, Minecraft minecraft) {
        ...
        String risky = modules.all().stream()
                ...
                .filter(module -> module.risk() != dev.sealedclient.core.ModuleRisk.PASSIVE)
                .map(Module::id)
                .collect(Collectors.joining(", "));
        ...
        info("Engines: crystal " + crystal
                ...
                + (navigation.ownedBySealed() ? " (owned)" : ""));
        return true;
    }
        ...
        return switch (action) {
            case "status" -> {
                ...
                info("Baritone" + version + ": " + status.state()
                        ...
                        + retries
                        + elapsed
                        + (status.ownedBySealed() ? " • owned by Sealed Client" : ""));
                yield true;
            }
            case "pause" -> {
                requireBaritoneSuccess(baritone.pause());
                info("Sealed-owned Baritone navigation paused");
                yield true;
            }
            case "resume" -> {
                requireBaritoneSuccess(baritone.resume());
                info("Sealed-owned Baritone navigation resumed");
                yield true;
            }
            case "stop" -> {
                requireBaritoneSuccess(baritone.stop());
                info("Sealed-owned Baritone navigation stopped");
                yield true;
            }
    ...
    private static void help(Minecraft minecraft) {
        ...
        info("Example: ;sealed toggle auto_totem");
    }

    private static void info(String message) {
        output("§b[Sealed] §f" + message);
    }

    private static void error(String message) {
        output("§c[Sealed] " + message);
    }
